...
class CreateURLScreen extends StatefulWidget {
  const CreateURLScreen({Key? key}) : super(key: key);
...
class _CreateURLScreenState extends State<CreateURLScreen> {
  ...
  Widget icon = const Icon(Icons.link, color: Colors.white);
  String name = "URL";
  Mat.Color color = Colors.grey;

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text("Create URL Barcode"),
        ),
        body: ListView(
            children: [
        Column(
    ...
    child: Form(
      ...
      validator: (value) => hasValidUrl(value) ? null : "Please enter a valid URL",
    ...
    onPressed: () {
    setState(() {
    if(myController.text.isNotEmpty && hasValidUrl(myController.text)) {
    generatedText = '${myController.text}';
    db.genBarcodes?.add(Barcodes(
    content: generatedText, type: "URL"));
    db.updateDatabase();
    }
    else{
